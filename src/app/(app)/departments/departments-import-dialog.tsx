  ...
  async function handleFile(files: File[]) {
    ...
    try {
      ...
      if (json.data.created > 0) {
        toast.success(`Imported ${json.data.created} of ${json.data.total} department(s)`);
        router.refresh();
      }
    } finally {
      setUploading(false);
    ...
    >
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an .xlsx file with a &quot;Name&quot; header row — the same column Export produces, so
            exporting once gives you a ready-made template. Existing department names are rejected, not
            overwritten.
          </DialogDescription>
        </DialogHeader>
                    ...
                    <tr key={result.row} className="border-b last:border-0">
                      ...
                      <td className="px-3 py-1.5">
                        {result.status === "created" ? (
                          <span className="text-green-700 dark:text-green-500">Created {result.name}</span>
                        ) : (
