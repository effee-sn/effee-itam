...
import { parseDepartmentImportFile, importDepartmentRows } from "@/modules/departments/import";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = await parseDepartmentImportFile(buffer);
    if ("error" in parsed) {
    ...
    const summary = await importDepartmentRows(parsed.rows, session.userId);
