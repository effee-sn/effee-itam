...
import { parseAssetImportFile, importAssetRows } from "@/modules/assets/import";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = await parseAssetImportFile(buffer);
    if ("error" in parsed) {
    ...
    const summary = await importAssetRows(parsed.rows, session.userId);
